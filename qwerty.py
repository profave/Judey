...
logo = (f''' 
      \033[1;95m          __       _
      \033[1;95m       __|  │__ _ | |___ 
      \033[1;95m      |     │ | | | | ‐_|
      \033[1;95m      |_____|___|___|___|{r} \n''')
...
def overview():
    ...
    print(f"│  {r}     TOTAL ACCOUNTS: {mage}{total_accounts}{r}              │")
    bru()
...
def menu():
    ...
    for (left_option, left_desc), (right_option, right_desc) in zip(left_options, right_options):
        print(f"│  {mage}->{r}{left_option} {r} {left_desc:<14}     {mage}->{r}{right_option}{r} {right_desc}   │")
    bro()
    choice = input(f'   ╰─>{mage}Select')
    menu_actions = {
        '1': Initialize,
        '2': QuickReact,
        '4': lambda: print('Thank you for using the tool') or exit()
    }
...
def Manual():
    user = input(f"{mage}USER ID/EMAIL:{wh} ")
    passw = input(f"{mage}PASSWORD:{wh} ")
...
def Auto():
    ...
    if not txt_files:
        print(f'\033[1;95m no .txt files found in {directory}')
        return
    
    for i, filename in enumerate(txt_files, start=1):
        print(f"   {mage}{i}  {wh}  {filename}")
...
def Initialize():
    ...
    linex()
    print(f"│{mage}     ->{r}1 {r} Automatic through option      │")
    bro()
...
def Reaction(actor_id:str, post_id:str, react:str, token:str):
    ...
    try:
        if react in str(pos): print(f"{mage}->Success")
        else: print(f"{red}->Failed")
    except Exception: print('')

def choose_reaction():
    print(f"{mage} select number from 1 to 7\n")
    print(f"{mage}     ->{r} 1 Like")
    print(f"{mage}     ->{r} 2 Love")
    print(f"{mage}     ->{r} 3 Haha")
    print(f"{mage}     ->{r} 4 Wow")
    print(f"{mage}     ->{r} 5 Care")
    print(f"{mage}     ->{r} 6 Sad")
    print(f"{mage}     ->{r} 7 Angry")
